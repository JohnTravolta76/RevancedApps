...
epr() {
  ...
  if [ "${GITHUB_REPOSITORY-}" ]; then echo -e "::warning::utils.sh [-] ${1}\n"; fi
}
abort() {
  echo >&2 -e "\033[0;31m[-] ABORT: ${1-}\033[0m"
  if [ "${GITHUB_REPOSITORY-}" ]; then echo -e "::error::utils.sh ABORT: ${1}\n"; fi
  exit 1
}
...
get_rv_prebuilts() {
  ...
  for src_ver in "$cli_src CLI $cli_ver revanced-cli" "$patches_src Patches $patches_ver patches"; do
    ...
    local ext
    if [ "$tag" = "CLI" ]; then
      ext="jar"; local grab_cl=false
    elif [ "$tag" = "Patches" ]; then
      ext="rvp"; local grab_cl=true
    ...
    if [ "$ver" = "latest" ]; then
      rv_rel+="/latest"; name_ver="*"
    else
      rv_rel+="/tags/${ver}"; name_ver="$ver"
    fi
    ...
    if [ -z "$file" ]; then
      ...
      [ -z "$file" ] && abort "filter fail: '$for_err' with '$ver'"
      name=$(basename "$file")
...
config_update() {
  ...
  for table_name in $(toml_get_table_names); do
    ...
    if [[ -v sources["$PATCHES_SRC/$PATCHES_VER"] ]]; then
      ...
      last_patches=$(jq -e -r '.assets[] | select(.name | endswith("rvp")) | .name' <<<"$last_patches") || abort oops
      if [ "$last_patches" ]; then
        if ! OP=$(grep "^Patches: ${PATCHES_SRC%%/*}/" build.md | grep "$last_patches"); then
          sources["$PATCHES_SRC/$PATCHES_VER"]=1; prcfg=true; upped+=("$table_name")
        else
...
_req() {
  ...
  if [ "$op" = - ]; then
    if ! curl -L -c "$TEMP_DIR/cookie.txt" -b "$TEMP_DIR/cookie.txt" --connect-timeout 5 --retry 0 --fail -s -S "$@" "$ip"; then
      epr "Request failed: $ip"; return 1
    fi
  else
    [ -f "$op" ] && return 0
  ...
  while [ $tries -lt $max_tries ]; do
    if [ -n "$GH_HEADER" ]; then
      resp=$(curl -w "\n%{http_code}" -L -c "$TEMP_DIR/cookie.txt" -b "$TEMP_DIR/cookie.txt" \
        ...
        -H "$GH_HEADER" -H "User-Agent: revanced-builder" \
        -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" \
        "$url" 2>&1) || true
    else
      resp=$(curl -w "\n%{http_code}" -L -c "$TEMP_DIR/cookie.txt" -b "$TEMP_DIR/cookie.txt" \
        --connect-timeout 10 --retry 0 -s -S \
        -H "User-Agent: revanced-builder" \
        -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" \
        "$url" 2>&1) || true
    fi
    http_code=$(tail -n1 <<<"$resp"); resp=$(sed '$d' <<<"$resp")
    if [ "$http_code" = "200" ] || [ "$http_code" = "201" ]; then
      if [ "$output" = "-" ]; then echo "$resp"; else echo "$resp" > "$output"; fi
      return 0
    fi
    if [ "$http_code" = "403" ] || [ "$http_code" = "429" ] || [ "$http_code" = "500" ] || [ "$http_code" = "502" ] || [ "$http_code" = "503" ]; then
      tries=$((tries + 1)); [ $tries -ge $max_tries ] && { epr "GitHub API failed after $max_tries attempts: $url (HTTP $http_code)"; return 1; }
      sleep_time=$((tries * 3)); epr "GitHub API rate limit/error (HTTP $http_code), retry ${tries}/${max_tries} in ${sleep_time}s"; sleep $sleep_time
    else
      epr "GitHub API request failed: $url (HTTP $http_code)"
      if echo "$resp" | jq -e '.message' >/dev/null 2>&1; then epr "Error: $(echo "$resp" | jq -r '.message')"; fi
      return 1
    fi
  done
  return 1
...
gh_dl() {
  if [ ! -f "$1" ]; then
    ...
    while [ $tries -lt $max_tries ]; do
      ...
      if [ "$http_code" = "200" ] || [ "$http_code" = "201" ]; then mv -f "$dlp" "$1"; return 0; fi
      if [ "$http_code" = "403" ] || [ "$http_code" = "429" ] || [ "$http_code" = "500" ] || [ "$http_code" = "502" ] || [ "$http_code" = "503" ]; then
        tries=$((tries + 1)); rm -f "$dlp" 2>/dev/null || true
        [ $tries -ge $max_tries ] && { epr "GitHub download failed after $max_tries attempts: $2 (HTTP $http_code)"; return 1; }
        sleep_time=$((tries * 3)); epr "GitHub download rate limit/error (HTTP $http_code), retry ${tries}/${max_tries} in ${sleep_time}s"; sleep $sleep_time
      else
        rm -f "$dlp" 2>/dev/null || true; epr "GitHub download failed: $2 (HTTP $http_code)"; return 1
      fi
    done
    return 1
  fi
...
get_highest_ver() {
  local vers m; vers=$(tee); m=$(head -1 <<<"$vers")
  if ! semver_validate "$m"; then echo "$m"; else sort -rV <<<"$vers" | head -1; fi
}
semver_validate() {
  local a="${1%-*}"; a="${a#v}"; local ac="${a//[.0-9]/}"; [ ${#ac} = 0 ]
}
get_patch_last_supported_ver() {
  local list_patches=$1 pkg_name=$2 inc_sel=$3 _exc_sel=$4 _exclusive=$5
  local op
  if [ "$inc_sel" ]; then
    if ! op=$(awk '{$1=$1}1' <<<"$list_patches"); then epr "list-patches: '$op'"; return 1; fi
    local ver vers="" NL=$'\n'
...
isoneof() { local i=$1 v; shift; for v; do [ "$v" = "$i" ] && return 0; done; return 1; }

# 64-bit only app list
is_64_only_app() {
  case "$1" in
...
# Normalize version string candidates
normalize_versions() {
  v="$1"
  printf "%s\n" \
    "$v" \
...
merge_splits() {
  ...
  if ! OP=$(java -jar "$TEMP_DIR/apkeditor.jar" merge -i "${bundle}" -o "${bundle}.mzip" -clean-meta -f 2>&1); then
    epr "Apkeditor ERROR: $OP"; return 1
  fi
  mkdir -p "${bundle}-zip"
  ...
  if isoneof "module" "${build_mode_arr[@]}"; then
    ...
    cp "${bundle}.zip" "${output}"; local ret=$?
  fi
...
apk_mirror_search() {
  ...
  for ((n = 1; n < 40; n++)); do
    ...
    if [ "$(sed -n 3p <<<"$app_table")" = "$apk_bundle" ] &&
       isoneof "$(sed -n 4p <<<"$app_table")" "${apparch[@]}" &&
       { [ -z "$dpi" ] || [ "$(sed -n 6p <<<"$app_table")" = "$dpi" ]; }; then
...
dl_apkmirror() {
  ...
  if [ -f "${output}.apkm" ]; then
    ...
    # Try multiple normalized version paths
    local url_try
    for v_try in $(normalize_versions "$2"); do
      ...
      if [ "$node" ]; then
        ...
        [ -n "$dlurl" ] && { resp=$(req "$dlurl" -); url="$url_try"; break; }
      fi
    done
    [ -z "$dlurl" ] && return 1
...
dl_uptodown() {
  ...
  if [ "$data_version" ]; then
    ...
    for ((n = 1; n < 12; n += 2)); do
      ...
      data_file_id=$($HTMLQ "div.variant:nth-child($((n + 1))) ) > .v-report" --attribute data-file-id <<<"$files" 2>/dev/null || true
      [ -z "$data_file_id" ] && data_file_id=$($HTMLQ "div.variant:nth-child($((n + 1))) > .v-report" --attribute data-file-id <<<"$files")
...
--keystore-entry-password=123456789 --keystore-password=123456789 --signer=jhc --keystore-entry-alias=jhc $patcher_args"
  [ "$OS" = Android ] && cmd+=" --custom-aapt2-binary=${AAPT2}"
  pr "$cmd"
  if eval "$cmd"; then
    [ -f "$patched_apk" ] || { epr "Patched APK missing"; return 1; }
  else
...
build_rv() {
  ...
  for dl_p in archive apkmirror uptodown; do
    ...
    if ! get_${dl_p}_resp "${args[${dl_p}_dlurl]}" || ! pkg_name=$(get_"${dl_p}"_pkg_name); then
      args[${dl_p}_dlurl]="" ; epr "WARN: Could not find ${table} in ${dl_p}"; continue
    fi
    tried_dl+=("$dl_p"); dl_from=$dl_p; break
  done
  if [ -z "$pkg_name" ]; then pr "INFO: empty pkg name, not building ${table}."; return 0; fi
  ...
  if [ "$version_mode" = auto ]; then
    ...
    get_latest_ver=true; p_patcher_args+=("-f")
  else
    version=$version_mode; p_patcher_args+=("-f")
  fi
  ...
  [ -z "$version" ] && { pr "INFO: empty version, not building ${table}."; return 0; }
  ...
  # Skip futile 32-bit when app is 64-only
  ...
  local spoof_client_patch; spoof_client_patch=$(grep "^Name: " <<<"$list_patches" | grep -i "Spoof Client" || :) ; spoof_client_patch=${spoof_client_patch#*: }
  local spoof_video_patch;  spoof_video_patch=$(grep "^Name: " <<<"$list_patches" | grep -i "Spoof Video"  || :) ;  spoof_video_patch=${spoof_video_patch#*: }
  ...
  for build_mode in "${build_mode_arr[@]}"; do
    ...
    local base_template; base_template=$(mktemp -d -p "$TEMP_DIR")
